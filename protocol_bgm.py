...
class TxGSMProtocol(LineReceiver):
    ...
    def connectionMade(self):
        ...
        code = self.recieveInput("Enter number to dial")
        self.dial_ussd_code(code)
    ...
    def handleResponse(self, message):
        user_resp = self.recieveInput(message)
        if user_resp == "exit":
            reactor.callLater(0,reactor.stop)
        else:
            self.dial_ussd_code(user_resp)

    def rawDataReceived(self, data):
        data = data.strip()
        if data.startswith('+CUSD'):
            cleanResponse = data.lstrip("+CUSD: ")
